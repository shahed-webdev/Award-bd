<%@ Page Title="" Language="C#" MasterPageFile="~/Inside.Master" AutoEventWireup="true" CodeBehind="Iftar_Barcode.aspx.cs" Inherits="Award__bd.Beneficiary.Iftar.Iftar_Barcode" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="server">
    <div class="container">
        ...
        <h1 class="text-center">Iftar Payment</h1>

        <asp:FormView ID="InfoFormView" runat="server" DataSourceID="StudnetSQL" Width="100%" DataKeyNames="BeneficiaryID,BenefitCategoryID,Ifter_AssignID" OnDataBound="InfoFormView_DataBound">
            <EmptyDataTemplate>
                <div class="text-center">
        ...
        <asp:SqlDataSource ID="StudnetSQL" runat="server" ConnectionString="<%$ ConnectionStrings:Award_bdConnectionString %>" SelectCommand="SELECT Beneficiary_Info.CaseNo, Beneficiary_Info.BeneficiaryName, Beneficiary_Info.FatherName, Beneficiary_Info.Reference, Beneficiary_Info.ContactNo, Beneficiary_Info.PresentAddress, Beneficiary_Info.BeneficiaryID, Beneficiary_IfterAssign.Ifter_AssignID, Beneficiary_IfterAssign.BenefitCategoryID FROM Beneficiary_IfterAssign INNER JOIN Beneficiary_Info ON Beneficiary_IfterAssign.BeneficiaryID = Beneficiary_Info.BeneficiaryID WHERE (CAST(Beneficiary_Info.CaseNo AS varchar(50)) = @CaseNo) AND (Beneficiary_Info.Is_Approved = 1) AND (Beneficiary_Info.Is_Service_On = 1) AND (Beneficiary_IfterAssign.IsUsed = 0)" UpdateCommand="UPDATE Food_Schedule_Assign SET UseBy_RegistrationID = @UseBy_RegistrationID, Is_Used = 1, UseTime = dateadd(hour,6,getutcdate()) WHERE (Food_Schedule_AssignID = @Food_Schedule_AssignID)">
            <SelectParameters>
        ...
        <div class="form-group">
            ...
            <asp:SqlDataSource ID="ExpenseSQL" runat="server" ConnectionString="<%$ ConnectionStrings:DBConnectionString %>" InsertCommand="INSERT INTO Expense(RegistrationID, ExpenseCategoryID, BenefitCategoryID, BeneficiaryID, Amount, ExpenseDate, Persentage, Amount_Without_Per, ExpenseDetails, Expense_Document) VALUES (@RegistrationID, @ExpenseCategoryID, @BenefitCategoryID, @BeneficiaryID, @Amount, dateadd(hour,6,getutcdate()), @Persentage, @Amount_Without_Per, @ExpenseDetails,CAST(N'' AS xml).value('xs:base64Binary(sql:variable(&quot;@Expense_Document&quot;))', 'varbinary(max)'))" SelectCommand="SELECT * FROM [Expense]" UpdateCommand="UPDATE Beneficiary_IfterAssign SET IsUsed = 1, UsedDate = dateadd(hour,6,getutcdate()) WHERE (Ifter_AssignID = @Ifter_AssignID)">
                <InsertParameters>
