<%@ Page Title="" Language="C#" MasterPageFile="~/Inside.Master" AutoEventWireup="true" CodeBehind="Beneficiary_Details_Approved.aspx.cs" Inherits="Award__bd.Beneficiary.Beneficiary_Details_Approved" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="server">
   <div id="B_Details">
      ...
      <asp:FormView ID="BeneficiaryFormView" runat="server" DataKeyNames="BeneficiaryID" DataSourceID="Beneficiary_InfoSQL" Width="100%">
         <ItemTemplate>
      ...
      <br />
      <asp:Button ID="ApproveButton" runat="server" Text="Approve" CssClass="btn btn-primary" OnClick="ApproveButton_Click" />
      <asp:Button ID="DeleteButton" runat="server" Text="Delete" CssClass="btn btn-primary" OnClick="DeleteButton_Click" />
      <asp:SqlDataSource ID="ApproveSQL" runat="server" ConnectionString="<%$ ConnectionStrings:DBConnectionString %>" SelectCommand="SELECT * FROM [Beneficiary_Info] WHERE  (BeneficiaryID = @BeneficiaryID)" UpdateCommand="UPDATE Beneficiary_Info SET Is_Approved = 1, ApprovedBy_ID =@ApprovedBy_ID, Is_Service_On = 1, CaseNo = dbo.Case_No() WHERE (BeneficiaryID = @BeneficiaryID)" DeleteCommand="DELETE FROM Beneficiary_Info WHERE (BeneficiaryID = @BeneficiaryID)
DELETE FROM [Beneficiary_Family] WHERE (BeneficiaryID = @BeneficiaryID)
DELETE FROM Beneficiary_BenefitCategory WHERE (BeneficiaryID = @BeneficiaryID)
DELETE FROM Beneficiary_Expense_Category WHERE (BeneficiaryID = @BeneficiaryID)">
          <DeleteParameters>
              <asp:QueryStringParameter Name="BeneficiaryID" QueryStringField="BeneficiaryID" />
          </DeleteParameters>
         <SelectParameters>
         ...
         <UpdateParameters>
            <asp:SessionParameter DefaultValue="" Name="ApprovedBy_ID" SessionField="RegistrationID" Type="Int32" />
            <asp:QueryStringParameter Name="BeneficiaryID" QueryStringField="BeneficiaryID" Type="Int32" />
         </UpdateParameters>
      </asp:SqlDataSource>
   </div>
